...
public class RLearner {
	...
	public RLearner( RLWorld world) {
		...
		RConsole.println("policy object with dimsize:" + dimSize.length +" created");
		//		
		// Initializing the policy with the initial values defined by the world.
		policy.initValues( thisWorld.getInitValues() );

		learningMethod = SARSA; //Q_LEARNING;  //Q_LAMBDA;//SARSA;
		actionSelection = E_GREEDY; //SOFTMAX
		...
		RConsole.println("RLearner initialised.");
	...
	public void runEpoch() {
		...
		thisWorld.resetState(state);
		RConsole.println("runEpoch(): State reset to length: "+state.length);
		
		
		switch( learningMethod ) {

		case Q_LEARNING : {
			RConsole.println("runEpoch(): in Q learning case");
			double this_Q;
			double max_Q;
			double new_Q;

			while( ! thisWorld.endState(state) ) { // continual improvement if no endState
				...
				state = newstate;
				RConsole.println("runEpoch(): Q learn iteration. reward: "+reward);
			}
