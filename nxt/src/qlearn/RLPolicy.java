...
//	double[] qValues;
	...
	RLPolicy( int[] dimSize ) {
		...
		// since reflect does not exist for Lejos, need to specialize here
		assert(dimSize.length == 3);
		// Create n-dimensional array with size given in dimSize array.
		//	qValuesTable = Array.newInstance( double.class, dimSize );
		qValuesTable = new double [dimSize[0]][dimSize[1]][dimSize[2]];
	...
	public void initValues( double initValue ) {
		...
		RConsole.println( "States: " + states);
		...
		for( int j = 0 ; j < states ; j++ ) {
			...
			for( i = 0 ; i < actions ; i++ ) {
				...
				qValuesTable[state[0]][state[1]][i] = initValue;
			}
...
//		Array.setDouble( qValues, action, newQValue );
		...
		qValuesTable[state[0]][state[1]][action]=newQValue;
	}
...
//		qValues = myQValues( state );
		double [] qValues = qValuesTable[state[0]][state[1]];
...
//		qValues = qValuesTable[state[0]][state[1]];
		...
		return qValuesTable[state[0]][state[1]][action];
	}
